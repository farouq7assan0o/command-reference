...
## Warning cleanup: group audit + uppercase placeholder autofill (2026-08-26)
Reviewed the validate warnings (none were failures - all gates already PASS). Fixed the two worthwhile ones:
1. GROUP AUDIT: added the 12 unmapped subcategories to js/groups.js GROUP_MAP (ColdFusion/DotNetNuke/osTicket -> Exploitation>Web Applications; Living off the Land/Security Controls/Windows Configuration/Network Discovery -> Enumeration>General; AV Bypass -> Exploitation>Shells; WordPress -> Web Exploitation>File Attacks; Login Brute Force -> Discovery&Brute Force; Nmap NSE -> VA>Scanners; new AD>Evasion group). Validate now: "every subcategory maps to a tree group" (no more "Other" fallback).
